...
1. Etablir une base des données résultante de la recherche à partir d'un panel d'entreprises
...
5. On peut déjà attribuer une valeur de 0 à 1 et une note pour un critère donnée
...
6. Note obtenue dans notre algorithme de chacune des entreprises ou la personne a travaillé.
7. Présence de mots clefs dans chaque recommandations.
8. Période d'innactivité.
...
Une pondération doit être appliqué à chaque critère pour que la note finale soit plus influencée par les critères
majeurs que par les critères mineures. La pondération peut être faite à la main dans un premier temps, en déterminant
de façon intuitives quels critères mettre en valeur.

La note finale peut être une moyenne des note établies pour chaque critère. On peut, ici encore pondérer les critères
...
neurones avec:
- en entrée la liste des différents résultats normalisée.
- en sortie la note finale.
Si on estime que la note d'une agence de notation est un cirtère statistiquement fiable pour juger de la solvabilité
d'une entreprise, on pourrrait entrainer le réseau de neurones à obtenir des résultats proches de ceux des agences de
notations.
Si on parvient à partir de critères qualitatifs dans la plupart des cas à une note équivalente de celle obtenue grâce à
des critères quantitatifs, on peut estimer que le modèle est bon.
...
- Quelle est la meilleure manière de normaliser les données issues du panel de test. Peut-être que la régression linéaire
n'est pas toujours le meilleur modèle. Apparemment pour être exploitables, la répartition des données normalisées doit
suivre une distribution normale. D'après ce que j'ai vu la détermination de la transformation à effectuer se fait
en essayant plusieurs methodes jusqu'à trouver celle qui donne une répartition normale:
